 ...
 */
class catalog_UpdateListAction extends change_Action
{
	/**
	...
	public function _execute($context, $request)
	{
		...
		{
			$ok = false;
			website_SessionMessage::addVolatileError($ls->trans('m.'.$module.'.fo.no-selected-product', $formatters));
		}
		else 
		{	
			$ok = true;		
			switch ($mode)
			{
				...
				default:
					$ok = false;
					website_SessionMessage::addVolatileError($ls->trans('m.'.$module.'.fo.unknown-update-mode', $formatters));
					break;
			}
		}
		...
		if ($ok)
		{
			...
			if ($listUrl)
			{
				...
				website_SessionMessage::addVolatileMessage($ls->trans('m.'.$module.'.fo.productlist-'.$name.'-updated-link', $formatters, $replacements));
			}
			else
			{
				website_SessionMessage::addVolatileMessage($ls->trans('m.'.$module.'.fo.productlist-'.$name.'-updated', $formatters));
			}
		}
