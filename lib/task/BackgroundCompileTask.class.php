...
class catalog_BackgroundCompileTask extends task_SimpleSystemTask
{
	...
	protected function execute()
	{
		...
		if ($toCompile == 0)
		{
			Framework::info('No product has marqued to recompile.');
			return;
		}
		Framework::info($toCompile .' products has marqued to recompile');
		...
		while ($maxIteration > 0 && $lastId >= 0)
		{
			...
			else
			{
				...
				if ($toCompile > 0)
				{
					$lastId = 0;
					$maxIteration--;
					Framework::info($toCompile .' products has marqued to recompile for iteration ' . (4 - $maxIteration));
				}
				else
				{
					$maxIteration = 0;
