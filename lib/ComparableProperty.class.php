	...
	public function getValueAsHtml($product)
	{
		switch ($this->prop->getType())
		{
			case BeanPropertyType::BOOLEAN:
				...
				return LocaleService::getInstance()->trans($key, array('ucf', 'html'));
...
class catalog_AvailabilityComparableProperty extends catalog_BaseComparableProperty
{
	...
	public function getLabel()
	{
		return LocaleService::getInstance()->trans('m.catalog.frontoffice.availability', array('ucf'));
	}
	...
	public function getValueAsHtml($product)
	{
		...
		{
			return LocaleService::getInstance()->trans('m.catalog.fo.add-to-cart-unavaible', array('ucf', 'html'));
		}
...
class catalog_ReviewComparableProperty extends catalog_BaseComparableProperty
{
	...
	public function getLabel()
	{
		return LocaleService::getInstance()->trans('m.catalog.fo.reviews', array('ucf'));
	}
	...
	public function getValueAsHtml($product)
	{
		...
		if ($count)
		{
			...
			$value .= $ls->trans('m.catalog.fo.reviews-count', array('ucf', 'html'), array('count' => $count));
			$value .= '</a>';
			if ($this->withRating)
			{
				$value .= '<br />';
				$value .= $ls->trans('m.catalog.frontoffice.rating-average', array('ucf', 'lab', 'html')) . ' ';
				$rating = $product->getFormattedRatingAverage();
				$value .= ($rating != '0.0') ? $rating : $ls->trans('m.catalog.fo.no-rating', array('ucf', 'html'));
			}
		}
		else
		{
			$value = $ls->trans('m.catalog.fo.no-review-yet', array('ucf', 'html'));
		}
		return $value;
	}
}
...
class catalog_ShortDescriptionComparableProperty extends catalog_BaseComparableProperty
{
	...
	public function getLabel()
	{
		return LocaleService::getInstance()->trans('m.catalog.document.product.description', array('ucf'));
	}
...
class catalog_PriceComparableProperty extends catalog_BaseComparableProperty
{
	...
	public function getLabel()
	{
		return LocaleService::getInstance()->trans('m.catalog.frontoffice.price'.$this->mode, array('ucf'));
	}
	...
	public function getValueAsHtml($product)
	{
		...
		$html .= ' <span class="tax-mode">' . LocaleService::getInstance()->trans('m.catalog.frontoffice.' . $this->mode, array('uc', 'html')) . '</span>';
		if ($price->isDiscount())
		{
