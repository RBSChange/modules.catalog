...
abstract class catalog_BlockProductBaseAction extends website_BlockAction
{
	...
	protected function addProductToFavorites($product)
	{
		...
		{
			$this->addMessage(LocaleService::getInstance()->trans('m.catalog.frontoffice.product-added-to-list', array('ucf')));
		}
	}
	...
	protected function addProductToCart($product)
	{
		...
		{
			$cart->refresh();
			$this->addMessage(LocaleService::getInstance()->trans('m.catalog.frontoffice.productjustadded', array('ucf')));
		}	
		else
		{
