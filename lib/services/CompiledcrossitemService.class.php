<?php
/**
 * @method catalog_CompiledcrossitemService getInstance()
 */
class catalog_CompiledcrossitemService extends f_persistentdocument_DocumentService
{
	...
	{
		return $this->getPersistentProvider()->createQuery('modules_catalog/compiledcrossitem');
	}
	...
	{
		return $this->getPersistentProvider()->createQuery('modules_catalog/compiledcrossitem', false);
	}
	...
	public function getSortingInfosByTargetAndLinkType($target, $linkType)
	{
		...
		foreach ($query->addOrder(Order::asc('position'))->find() as $item)
		{
			...
			$infos[] = array(
				'id' => $linked->getId(),
				'label' => $linked->getNavigationLabel(),
