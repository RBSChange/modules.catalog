 ...
 * @method catalog_BillingareaService getInstance()
 */
class catalog_BillingareaService extends f_persistentdocument_DocumentService
{
	...
	{
		return $this->getPersistentProvider()->createQuery('modules_catalog/billingarea');
	}
	...
	{
		return $this->getPersistentProvider()->createQuery('modules_catalog/billingarea', false);
	}
	...
	public function getCurrentBillingAreaForShop($shop, $refresh = false)
	{
		...
		{
			$data = change_Controller::getInstance()->getContext()->getStorage()->read('CurrentBillingAreaForShop');
	...
	public function setCurrentBillingAreaForShop($shop, $billingArea)
	{
		if ($shop instanceof catalog_persistentdocument_shop)
		{
			...
			change_Controller::getInstance()->getContext()->getStorage()->write('CurrentBillingAreaForShop', $data);
		}
		else
		{
			throw new Exception('No shop defined');
