jQuery(document).ready(function() {
	...
	jQuery('.products-comparison-list .short-description').attr('title', "${trans:m.catalog.fo.click-to-view-full-description,ucf,js}").click(function() { 
		jQuery(this).hide();
		jQuery(this).siblings('.full-description').show();
	});
	jQuery('.products-comparison-list .full-description').attr('title', "${trans:m.catalog.fo.click-to-hide-full-description,ucf,js}").click(function() { 
		jQuery(this).hide();
 ...
 * @param DOMNode node
 */
function onGlobalAddToFavoriteButtonClick(node)
{
	if (!node.form.hasAttribute('submitted'))
	{
		node.form.listName.value = 'catalog_favorite';
		jQuery(node.form).submit();
	}
}
